...
def extraer_digitos_centrales(numero: int, d: int) -> int:
    """Extrae los D dígitos centrales de 'numero' (rellenando con ceros
    a la izquierda si hace falta, como indican las diapositivas)."""
    numero_str = str(numero).zfill(2 * d)
    inicio = (len(numero_str) - d) // 2
    return int(numero_str[inicio:inicio + d])

# =====================================================================
# 1. ALGORITMOS NO CONGRUENCIALES
# =====================================================================

def productos_medios(semilla_0, semilla_1, d, n_iteraciones, detener_en_ciclo=True):
    resultados = []
    x_prev, x_actual = semilla_0, semilla_1
    vistos = {(x_prev, x_actual): 0}
    ciclo = None
    for i in range(1, n_iteraciones + 1):
        ...
        resultados.append({"iteracion": i, "X_i": x_prev, "X_i+1_usado": x_actual,
                            "Y_i": y_i, "X_i+2": x_siguiente, "r_i": round(r_i, 4)})
...
def congruencial_lineal(x0, a, c, m, n_iteraciones, detener_en_ciclo=True):
    ...
    ciclo = None
    for i in range(1, n_iteraciones + 1):
        x_actual = (a * x_actual + c) % m
        r_i = x_actual / (m - 1) if m > 1 else 0.0
        resultados.append({"iteracion": i, "X_i": x_actual, "r_i": round(r_i, 4)})
...
ALGORITMOS = {
    "1": {
        "id": "productos_medios", "grupo": "No congruencial",
        "nombre": "Productos medios",
        "formula": "Yi = Xi * Xi+1   ->   Xi+2 = dígitos centrales de Yi",
        "campos": [
            {"id": "semilla0", "label": "Semilla X0", "tipo": "entero", "defecto": 5015},
            {"id": "semilla1", "label": "Semilla X1", "tipo": "entero", "defecto": 5734},
            {"id": "d", "label": "Cantidad de dígitos D (>3)", "tipo": "entero", "defecto": 4},
        ],
        "columnas": [("iteracion", "i"), ("X_i", "Xi"), ("X_i+1_usado", "Xi+1"),
                     ("Y_i", "Yi"), ("X_i+2", "Xi+2"), ("r_i", "ri")],
    },
    "2": {
